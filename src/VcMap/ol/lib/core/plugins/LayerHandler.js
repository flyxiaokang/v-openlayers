 ...
 * @LastEditTime: 2023-08-16 14:06:39
 */
...
export default class LayerHandler {
  ...
  //     if (feature.geom) {
  //       const { geom, id } = feature
  //       if (feature.style) {
  //         _style = getStyle(feature.style)
  //       }
  //       const fs = new WKT().readFeatures(geom)
  //       const copyGeom = VcUtils.deepClone(feature)
  //       delete copyGeom.geom
  //       fs.forEach((f) => {
  //         f.setProperties(copyGeom)
  //         f.setStyle(_style)
  //         _features.push(f)
  //       })
  //     }
  //   })
  //   let layer = this.getLayerById(id, map)
  //   if (layer && clear) {
  ...
  createCustomLayer({
    ...
    features.forEach((feature, index) => {
      if (feature.geom) {
        const { geom, id = VcUtils.UUIDGenerator() } = feature
        if (feature.style) {
          _style = getStyle(feature.style)
        }
        if (multiple) {
          ...
          const f = new WKT().readFeature(geom)
          const copyFeature = VcUtils.deepClone(feature)
          delete copyFeature.geom
          if (id) {
            f.setId(id)
